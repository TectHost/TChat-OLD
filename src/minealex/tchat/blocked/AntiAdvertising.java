...
public class AntiAdvertising {
    ...
    public void handleBlockedIPv4(Player player) {
        String message = plugin.getMessage("antiAdvertisingIPv4Blocked");
        player.sendMessage(message);
    }

    public void handleBlockedDomain(Player player) {
        String message = plugin.getMessage("antiAdvertisingDomainBlocked");
        player.sendMessage(message);
    }

    public void handleBlockedLink(Player player) {
        String message = plugin.getMessage("antiAdvertisingLinkBlocked");
        player.sendMessage(message);
    }
}
