...
public class WarningCommand implements CommandExecutor {
    ...
    public WarningCommand(JavaPlugin plugin) {
        this.plugin = plugin;
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        if (args.length == 0) {
        	sender.sendMessage(getMessages("messages.warningUsage"));
            return true;
        }
        ...
        if (config.contains("Announcements.warningFormat")) {
            String format = config.getString("Announcements.warningFormat");

            String formattedMessage = ChatColor.translateAlternateColorCodes('&', format.replace("%s", message.toString().trim()));
            Bukkit.broadcastMessage(formattedMessage);
        } else {
            sender.sendMessage("Warning format not found in config.yml.");
        }
